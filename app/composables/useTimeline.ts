...
export function useTimeline() {
  const events = useState<TimelineEvent[]>('timeline-events', () => timelineDefaults as TimelineEvent[])
  ...
  const output = computed<TimelineOutput>(() => {
    ...
    const filingAdjustment = inputs.value.filingMethod === 'mail' ? 14 : inputs.value.filingMethod === 'in-person' ? -7 : 0

    const calculatedEvents = events.value.map((event) => {
      const adjustedOffset = event.type === 'edd' ? event.startOffset + filingAdjustment : event.startOffset
    ...
    const criticalDeadlines = calculatedEvents
      ...
      .map((event) => {
        ...
        const daysFromNow = Math.ceil((eventDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24))
        return {
          name: event.name,

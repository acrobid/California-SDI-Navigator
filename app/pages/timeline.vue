...
<template>
  <div class="container mx-auto px-4 py-8">
    <!-- Header -->
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Claim Timeline</h1>
      <p class="text-gray-600 dark:text-gray-400">
        Visualize your expected timeline based on your disability start date and filing method.
      </p>
    </div>
    ...
    <div class="grid lg:grid-cols-3 gap-8">
      ...
      <div>
          ...
          <div class="space-y-4">
            <UFormField label="Disability Start Date">
              <UInput
                ...
                type="date"
                @update:model-value="updateInput('claimStartDate', $event as string)"
              />
              <template #hint>
                Usually your last day worked
              </template>
            </UFormField>
            ...
            <UFormField label="Filing Method">
              <URadioGroup
                ...
                @update:model-value="updateInput('filingMethod', $event as 'online' | 'mail' | 'in-person')"
              />
              <template #hint>
            ...
            <UFormField>
              <UCheckbox
                ...
                @update:model-value="updateInput('livesInCalifornia', $event as boolean)"
              />
            </UFormField>
          </div>
        </UCard>
        ...
        <UCard class="mt-6">
          ...
          <div class="space-y-2">
            <div v-for="(label, type) in eventTypeLabels" :key="type" class="flex items-center gap-2">
              <span
      ...
      <div class="lg:col-span-2">
        ...
        <CommonCallout v-if="output.criticalDeadlines.some(d => d.daysFromNow <= 30 && d.daysFromNow >= 0)" variant="danger" title="Upcoming Deadlines">
          <ul class="list-disc list-inside space-y-1">
            <li v-for="deadline in output.criticalDeadlines.filter(d => d.daysFromNow <= 30 && d.daysFromNow >= 0)" :key="deadline.name">
              <strong>{{ deadline.name }}:</strong> {{ formatDateShort(deadline.date) }}
            ...
            <div class="space-y-6">
              ...
              >
                ...
                <div
                  ...
                  :class="{
                    'border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20': event.isCritical || event.type === 'deadline',
                    'border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800': !event.isCritical && event.type !== 'deadline',
                  }"
                >
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      <div class="flex items-center gap-2">
                        ...
                        <UBadge v-if="event.isCritical" color="red" variant="subtle" size="xs">
                          Critical
                        </UBadge>
                      </div>
                    ...
                    <div class="text-right flex-shrink-0">
                      ...
                      <p v-if="event.varianceDays > 0" class="text-xs text-gray-500">
        ...
        <UCard class="mt-6">
          ...
          <div class="space-y-3">
            <div
              ...
              :class="{
                'bg-red-50 dark:bg-red-900/20': deadline.daysFromNow <= 30 && deadline.daysFromNow >= 0,
                'bg-amber-50 dark:bg-amber-900/20': deadline.daysFromNow > 30 && deadline.daysFromNow <= 60,
                'bg-gray-50 dark:bg-gray-800': deadline.daysFromNow > 60 || deadline.daysFromNow < 0,
              }"
            >
              <div>
                <p class="font-medium text-gray-900 dark:text-white">{{ deadline.name }}</p>
                <p class="text-sm text-gray-500">{{ formatDateShort(deadline.date) }}</p>
              </div>
              <UBadge
                :color="deadline.daysFromNow < 0 ? 'neutral' : deadline.daysFromNow <= 30 ? 'red' : deadline.daysFromNow <= 60 ? 'orange' : 'green'"
                variant="subtle"
              >
                {{ deadline.daysFromNow < 0 ? 'Passed' : `${deadline.daysFromNow} days` }}
              </UBadge>
            </div>
          </div>
        </UCard>
    ...
    <div class="mt-8">
      <CommonCallout variant="info" title="Timeline Estimates">
        These timelines are estimates based on typical processing times. Actual timelines can vary significantly based on EDD workload, completeness of your application, and other factors.
      </CommonCallout>
    </div>
  </div>
</template>
